import { CarouselText, DesignerTitle, ImagePath } from '@/Constant/constant';
  ...
  const slides = CarouselDataList.map((item, index) => {
    return (
      <CarouselItem onExiting={() => setAnimating(true)} onExited={() => setAnimating(false)} key={index}>
        <picture><img src={`${ImagePath}/${item.image}`} alt='office-work' className='d-block w-100' /></picture>
      </CarouselItem>
    );
  });
